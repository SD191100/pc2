...
import { createVm, destroyVm, getVm, getVmState, listVms } from "./pulumi/pulumi.js";
...
app.post("/compute", (req, res) => {
    ...
    const { name, id, cpu, memory, storage, username, password, sshKey, templateId, gateway, ioAddress } = req.body;
    if (id == null ||
        name == null ||
        cpu == null ||
        memory == null ||
...
app.get(`/compute/vms/:vmId`, (req, res) => {
...
app.get(`/compute/vms`, async (req, res) => {
    try {
...
app.delete(`/compute/vms/:vmId`, (req, res) => {
    const { vmId } = req.params;
    if (vmId == null) {
